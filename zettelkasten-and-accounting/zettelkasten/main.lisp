...
(defun select-constrained-notes (&key ((:parameters parameters)))
  (list :parameters parameters
        :rows (apply #'zac.box.travel:select-notes-through-links *current-note* parameters)))


(defun single-note-picker (&key ((:selection selection)))
...
;        ))
        ...
        (add-shell-subexpressions
          shell
          ;(:next-designator ((:next-designator . "next"))
          (:optional-direction (((:type . "f(?:orward)?|b(?:ack(?:ward)?)?") :optional))
                               ...
                               "Parameter for operation direction.")
          (:optional-exponent (((:exponent . "[1-9]\\d*") :optional :optionally-immediate))
                              (lambda (&key ((:exponent exponent)))
                                (if exponent
                                  (parse-integer exponent)
                                  1))
                              "Parameter for number of times operation would be applied.")
          (:optional-closure (((:closure . "\\*") :optional :optionally-immediate))
                             (lambda (&key ((:closure closure)))
                               (not (null closure)))
                             "Parameter to enable concatenation of results of repeated calls to operation (Kleene star subset).")
          (:dae ((:direction . :optional-direction)
                 (:exponent . :optional-exponent)
                 (:closure . :optional-closure))
                (lambda (&key ((:direction direction)) ((:exponent exponent)) ((:closure closure)))
           ...
           (:single-constrained-note-picker ((:selection . :constrained-note-selector))
                                            #'single-note-picker
        ...
        (add-shell-commands
          shell
          (("home") #'command-home "Go to root note")
          ((:many-forms ("next") ("n")) #'command-next "Goto next note in sequence")
          (("g(?:oto)?" (:note . :single-constrained-note-picker)) (lambda (&key ((:note note-id)))
                                                                     (when note-id
