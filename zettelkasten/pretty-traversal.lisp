...
(in-package :zettelkasten.pretty-traversal)
...
(defun column-names-for-notes-through-links (backward? exponent closure?)
  ...
  (let ((show-numbers? (and (not closure?)
    ...
    (list-existing (when closure? "Referrers")
                   (second (assoc :text zk.db:+table-note-fields+))
                   (when show-numbers? (second (assoc :number zk.db:+table-link-fields+))))))
...
;  (find-row-with-peeking-dialog (get-field-names zk.db:+table-note-fields+)
;                                rows
;                                :choose-many choose-many
